...
- Keep native mode free of CDP and keep the full-bar debugging endpoint bound to `127.0.0.1`.
- Preserve the distinction between active `last_token_usage` and cumulative `total_token_usage`.
- Include the ChatGPT desktop version and `scripts/doctor-native.sh` output for native-mode reports. Include `scripts/doctor.sh` output when reporting full-bar selector or startup recovery failures.
